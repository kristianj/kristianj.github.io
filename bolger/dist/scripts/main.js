...
class BasicKnob {
	...
	constructor(input) {
		...
		let settings = this.settings = this._getSettings(input);
		...
		container.addEventListener('mousedown', this._handleMove.bind(this), false);
	...
	_handleMove(event) {
		...
		let fnc = this._updateWhileMoving.bind(this);
		window.addEventListener('mousemove', fnc, false);
		window.addEventListener('mouseup', () => {
			window.removeEventListener('mousemove', fnc, false);
		}, false);
	}

	_updateWhileMoving(event) {
		event.preventDefault();
		let delta = ((this.settings.max - this.min) / this.sliderRange) * (this.prevY - event.pageY);
		this.prevY = event.pageY;
		this.update(this.limit(delta + this.input.valueAsNumber));
		this.triggerChange();
	}
...
var PolyOscillator = function() {
	var freq;
	var time;
	var sampleRate = Pizzicato.context.sampleRate;
	...
	var osc = function(e) {
		...
		for (var i = 0; i < e.outputBuffer.length; i++) {
			t = (2 * Math.PI) * (time/sampleRate);
			output[i] = 0;
			//env = time < attackTime ? (time / attackTime) : Math.exp(decay*((2 * Math.PI) * ((time-attackTime)/sampleRate))*decayFactor);
			env = Math.exp(decay*t*decayFactor);
			for (var o=0; o<oscillators.length; o++) {
				output[i] += oscillators[o].level*Math.sin((oscillators[o].period+oscillators[o].tune) * freq * t) * env;
			}
			if (noise) {
				output[i] += noise * Math.random() * env;
			}
			output[i] = Math.pow(output[i], mul)*gain;
			if (output[i] > limit) {
				output[i] = limit;
			}
			if (output[i] < negLimit) {
				output[i] = negLimit;
			}
			time++;
		}
	...
	var panner = new Pizzicato.Effects.StereoPanner({
		pan: (Math.random()*panAmount)-(panAmount/2)
	});

	var sound = new Pizzicato.Sound(osc);
	sound.addEffect(panner);

	this.getPreview = function(f) {
		freq = f;
		time = 0;
		mul = multiply;
		var helper = {
	...
	this.setOscillatorParam = function(index, param, value) {
		oscillators[index][param] = value;
	};
	this.setMultiply = function(m) {
		multiply = m;
		if (m == 1) {
			decay = -0.5;
		}
	...
	this.setDecayFactor = function(v) {
		decayFactor = v;
	};
	this.setGain = function(v) {
		gain = v;
	};
	this.setNoise = function(n) {
		noise = n;
	};
	this.setPan = function(p) {
		if (p <= 2 && p >= 0) {
			panAmount = p;
			panner.pan = (Math.random()*panAmount)-(panAmount/2);
		}
	};
	...
	this.getPatchData = function() {
		...
		for (var i=1; i<=oscillators.length; i++) {
			data['osc-level-'+i] = oscillators[i-1]['level'];
			data['osc-period-'+i] = oscillators[i-1]['period'];
			data['osc-tune-'+i] = oscillators[i-1]['tune'];
		}
		return data;
	};
	this.setPatchData = function(data) {
		decayFactor = data.decay;
		multiply = data.multiply;
		gain = data.gain;
		panAmount = (data.pan !== undefined) ? data.pan : 2;
		noise = (data.noise !== undefined) ? data.noise : 0;
		limit = (data.limit !== undefined) ? data.limit : 3;

		for (var i=1; i<=oscillators.length; i++) {
			oscillators[i-1]['level'] = data['osc-level-'+i];
			oscillators[i-1]['period'] = data['osc-period-'+i];
			oscillators[i-1]['tune'] = data['osc-tune-'+i];
		}
	};
};
class PolySynth {
	...
	constructor(poly) {
		this.tones = [];
		this.currentTone = 1;	// oscillator currently playing
		this.scale = [];
		this.group = new Pizzicato.Group();
		this.notes = {}; // notes currently playing

		for (let i=0; i<=poly; i++) {
			this.tones[i] = new PolyOscillator();
			this.group.addSound(this.tones[i].getSound());
		}

		this.delay = new Pizzicato.Effects.PingPongDelay({
			feedback: 0.4,
			time: 0.45,
			mix: 0.1
		});

		this.group.addEffect(this.delay);
		...
		this.group.addEffect(this.reverb);

		// create lookup table for midi note to frequency
		for (let i=0; i<128; i++) {
			this.scale[i] = 440 * Math.pow(2, (i - 69) / 12);
		}
	...
	noteOn(n) {
		n = parseInt(n, 10);
		if (this.scale[n] && !this.notes[this.scale[n]]) {
			this.notes[this.scale[n]] = true;
			this.tones[this.currentTone].play(this.scale[n]);
			if (++this.currentTone >= this.tones.length) {
				this.currentTone = 1;	// tone 0 is reserved for preview
			}
		}
	}
	...
	setOscillatorParam(index, param, value) {
		for (const tone of this.tones) {
			tone.setOscillatorParam(index, param, value);
		}
	}

	getWaveForm() {
		return this.tones[0].getPreview(220);
	}
	...
	getPatch() {
		let data = this.tones[0].getPatchData();

		data.delayfeedback = this.delay.feedback;
		data.delaytime = this.delay.time;
		data.delaymix = this.delay.mix;

		data.reverbtime = this.reverb.time;
		data.reverbdecay = this.reverb.decay;
		data.reverbmix = this.reverb.mix;
	...
	loadPatch(data) {
		for (const tone of this.tones) {
			tone.setPatchData(data);
		}
		this.delay.feedback = data.delayfeedback;
		this.delay.time = data.delaytime;
		this.delay.mix = data.delaymix;

		this.reverb.time = data.reverbtime;
		this.reverb.decay = data.reverbdecay;
		this.reverb.mix = data.reverbmix;
	}

	onMIDIMessage(message) {
		let data = message.data;

		let type = data[0] & 0xf0; // Channel agnostic type
		...
		else if (type === 176 && data[1] === 1) { // mod wheel
			let value = (data[2] / 127)*6;
			console.log(value);
			// $('#sparkle').val(value*100).trigger('change');
			this.multiply = Math.ceil(value);
		}
	}
...
/*
	this.toggleDelay = function(status) {
		if (status) {
	...
	this.toggleReverb = function(status) {
		if (status) {
...
class PolySynthUI {
	...
	constructor() {
		// keys on the keyboard that trigger notes
		this.keyMap = {
			90: 50,
			88: 52,
			67: 53,
		...
		this.synth = new PolySynth(10);
		this.synth.multiply = 2;

		document.addEventListener('keydown', (e) => {
			if (this.keyMap[e.keyCode]) {
				this.synth.noteOn(this.keyMap[e.keyCode]);
			}
		});
		document.addEventListener('keyup', (e) => {
			if (this.keyMap[e.keyCode]) {
				this.synth.noteOff(this.keyMap[e.keyCode]);
			}
		});

		this._get('.control label').forEach(el => {
			el.addEventListener('click', (e) => {
				e.preventDefault();
				var knob = this.knobs[el.getAttribute('for')];
				knob.update(knob.input.dataset.orgvalue);
			});
		});

		this._get('.osc-param').forEach(el => {
			el.dataset.orgvalue = el.value;
			this.makeKnob(el, () => { 
				let osc = parseInt(el.dataset.osc, 10);
				let param = el.dataset.param;
				let round = el.dataset.round;
				let value = el.valueAsNumber;

				if (round) {
					value = Math.round(value);
				}
				this.synth.setOscillatorParam(osc, param, value);
				this.updatePreview();
			});
		});

		this._get('.synth-param').forEach(el => {
			this.makeKnob(el, () => { 
				this.synth[el.dataset.param] = el.value;
				this.updatePreview();
			});
		});

		this._get('.delay-param').forEach(el => {
			this.makeKnob(el, () => { this.synth.delay[el.dataset.param] = +el.value });
		});

		this._get('.reverb-param').forEach(el => {
			this.makeKnob(el, () => { this.synth.reverb[el.dataset.param] = +el.value });
		});

		this._get('.noise-param').forEach(el => {
			this.makeKnob(el, () => { 
				this.synth.noise = +el.value;
				this.updatePreview();
			});
		});
		this._get('.pan-param').forEach(el => {
			this.makeKnob(el, () => { 
				this.synth.pan = +el.value;
			});
		});
		this._get('.limit-param').forEach(el => {
			this.makeKnob(el, () => { 
				this.synth.limit = +el.value;
				this.updatePreview();
			});
		});		
	...
	makeKnob(el, onChange) {
		el.dataset.width = 80;
		el.dataset.height = 80;
		el.addEventListener('change', onChange);
		this.knobs[el.id] = new Knob(el);
	}

	updatePreview() {
		let data = this.synth.getWaveForm();
		let i;
		let max = 2;
		let maxVel = 0;
	...
	getPatch() {
		return JSON.stringify(this.synth.getPatch());
	}
	setPatch(input) {
		for (let k in this.knobs) {
			if (input[k] !== undefined) {
				this.knobs[k].update(input[k]);
			}
		}
	}
	loadPatch(index) {
	...
	renderPatches() {
		...
		for (let p in patches) {
			patch = patches[p];
			preset = document.createElement('a');
			preset.addEventListener('click', this.setPatch.bind(this, patch.data));
			preset.innerText = patch.name;
...
Knob = function(input) {
  ...
  this.ui.init(container, settings);
  this.container = container;
  this.changed(0);
...
Knob.prototype = {
  ...
  _updateWhileMoving: function(event) {
    event.preventDefault();
    var delta = ((this.settings.max - this.min) / this.sliderRange) * (this.orgY - event.pageY);
    this.orgY = event.pageY;
    var newVal = this.limit(delta + this.input.valueAsNumber);
    this.update(newVal);
    this.triggerChange();
  },
...
KnobUI.prototype = Object.create(Ui.prototype);

KnobUI.prototype.createElement = function() {
...
var arc = new Ui.El.Arc(this.merge(this.options, {arcWidth: this.width / 5}));
arc.setAngle(this.options.anglerange);
this.el.node.appendChild(arc.node);
    this.el.node.setAttribute("class", "knob");
};
var patches = [{"name":"0 default","data":{"multiply":2,"decay":"1","gain":"0.5","osc-level-1":0.6,"osc-period-1":1,"osc-tune-1":0,"osc-level-2":0.4,"osc-period-2":2,"osc-tune-2":0,"osc-level-3":0.6,"osc-period-3":2,"osc-tune-3":0,"delayfeedback":0.4,"delaytime":0.45,"delaymix":0.1,"reverbtime":1,"reverbdecay":1,"reverbmix":0.5}},{"name":"ad noiseum","data":{"multiply":5,"decay":"1.52","gain":"1","noise":0.04,"pan":2,"limit":3.8,"osc-level-1":0.6,"osc-period-1":3,"osc-tune-1":0,"osc-level-2":0.4,"osc-period-2":2,"osc-tune-2":0,"osc-level-3":0.6,"osc-period-3":3,"osc-tune-3":0,"delayfeedback":0.4,"delaytime":0.45,"delaymix":0.1,"reverbtime":1,"reverbdecay":0.35,"reverbmix":0.5}},{"name":"deeptremor","data":{"multiply":3,"decay":"1.04","gain":"0.25","osc-level-1":0.92,"osc-period-1":1,"osc-tune-1":-0.5,"osc-level-2":0.9,"osc-period-2":1,"osc-tune-2":0,"osc-level-3":0.6,"osc-period-3":3,"osc-tune-3":-0.48,"delayfeedback":0.4,"delaytime":0.45,"delaymix":0,"reverbtime":1,"reverbdecay":0.1,"reverbmix":0.01}},{"name":"fiskergaard","data":{"multiply":1,"decay":"0.48","gain":"0.36","noise":0,"pan":0.36,"limit":6,"osc-level-1":0.6,"osc-period-1":2,"osc-tune-1":0,"osc-level-2":0.4,"osc-period-2":3,"osc-tune-2":0.24,"osc-level-3":0.8,"osc-period-3":3,"osc-tune-3":-0.92,"delayfeedback":0.1,"delaytime":0.63,"delaymix":0.86,"reverbtime":2.1,"reverbdecay":1.1,"reverbmix":0.65}},{"name":"gritty chimes","data":{"multiply":6,"decay":"0.6","gain":"0.78","osc-level-1":0.12,"osc-period-1":2,"osc-tune-1":0,"osc-level-2":0,"osc-period-2":2,"osc-tune-2":0,"osc-level-3":0.96,"osc-period-3":3,"osc-tune-3":0,"delayfeedback":0.4,"delaytime":0.21,"delaymix":0.32,"reverbtime":2.25,"reverbdecay":2.1,"reverbmix":0.5}},{"name":"mellow","data":{"multiply":3,"decay":"1","gain":"0.62","osc-level-1":0.6,"osc-period-1":2,"osc-tune-1":0,"osc-level-2":0.4,"osc-period-2":2,"osc-tune-2":0,"osc-level-3":0.6,"osc-period-3":2,"osc-tune-3":0.04,"delayfeedback":0.4,"delaytime":0.45,"delaymix":0.1,"reverbtime":1,"reverbdecay":1,"reverbmix":0.5}},{"name":"overtones","data":{"multiply":6,"decay":"2","gain":"0.62","osc-level-1":0.6,"osc-period-1":2,"osc-tune-1":-0.32,"osc-level-2":0.4,"osc-period-2":2,"osc-tune-2":0,"osc-level-3":0.6,"osc-period-3":2,"osc-tune-3":0.36,"delayfeedback":0.4,"delaytime":0.45,"delaymix":0.1,"reverbtime":1,"reverbdecay":1,"reverbmix":0.5}},{"name":"overtones 2","data":{"multiply":6,"decay":"2","gain":"0.62","osc-level-1":0.6,"osc-period-1":4,"osc-tune-1":-0.6,"osc-level-2":0,"osc-period-2":3,"osc-tune-2":0,"osc-level-3":0.6,"osc-period-3":4,"osc-tune-3":-0.4,"delayfeedback":0.4,"delaytime":0.45,"delaymix":0.38,"reverbtime":1,"reverbdecay":1,"reverbmix":0.39}},{"name":"percussive hit","data":{"multiply":6,"decay":"2","gain":"0.62","osc-level-1":0.6,"osc-period-1":2,"osc-tune-1":-0.32,"osc-level-2":0.4,"osc-period-2":2,"osc-tune-2":0,"osc-level-3":0.6,"osc-period-3":2,"osc-tune-3":0.36,"delayfeedback":0.4,"delaytime":0.45,"delaymix":0.1,"reverbtime":1,"reverbdecay":1,"reverbmix":0.5}},{"name":"ringmod","data":{"multiply":3,"decay":"0.56","gain":"0.5","osc-level-1":0.6,"osc-period-1":1,"osc-tune-1":-0.68,"osc-level-2":0.4,"osc-period-2":1,"osc-tune-2":-0.8,"osc-level-3":0.6,"osc-period-3":3,"osc-tune-3":0.16,"delayfeedback":0.4,"delaytime":0.45,"delaymix":0.1,"reverbtime":1,"reverbdecay":1,"reverbmix":0.5}},{"name":"shimmering","data":{"multiply":5,"decay":"0.4","gain":"0.5","osc-level-1":0.6,"osc-period-1":2,"osc-tune-1":0,"osc-level-2":0.9,"osc-period-2":2,"osc-tune-2":-0.04,"osc-level-3":0.01,"osc-period-3":2,"osc-tune-3":0,"delayfeedback":0.4,"delaytime":0.45,"delaymix":0.1,"reverbtime":1.4,"reverbdecay":1,"reverbmix":0.61}},{"name":"subbass","data":{"multiply":1,"decay":"1.32","gain":"0.88","osc-level-1":0.94,"osc-period-1":1,"osc-tune-1":-0.5,"osc-level-2":0.92,"osc-period-2":1,"osc-tune-2":0,"osc-level-3":0,"osc-period-3":2,"osc-tune-3":0,"delayfeedback":0.4,"delaytime":0.45,"delaymix":0.1,"reverbtime":1,"reverbdecay":1,"reverbmix":0.5}}];
